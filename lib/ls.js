...
/**
 * Compile a LispyScript S-expression to a Javascript function.
 *
 * Usage:
 *
 *   var ls = require("lispyscript");
 *
 *   var foo = ls.compile('(console.log "Hello, world!")');
 *   foo();
 *
 * To pass arguments provide an array of input variable names:
 *
 *   foo = ls.compile('(* a b)', ["a", "b"]);
 *   foo(2, 3);  // returns 6
 */
exports.compile = function (code, inputs) {
  if (inputs === undefined) {
    inputs = [];
  }

  // need to create a temporary variable called __lsMain since defining a top
  // level anonymous function is not valid Javascript.
  var code = exports._compile("(set __lsMain (function (" + 
      inputs.join(" ") + ") " + code + "))", "<eval>");
