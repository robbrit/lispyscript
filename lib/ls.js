 ...
 * foo();
 *
 * To get an output, you need to specify outputs and provide a callback:
 *
 * foo = ls.compile('(set a (* 2 3))', "a");
 * foo(function (a) {
 *   console.log(a);
 * });
 *
 * foo = ls.compile('(set a (* 2 3)) (set b (+ 100 200))', ["a", "b"]);
 * foo(function (a, b) {
 *   console.log(a, b);
 * });
 */
exports.compile = function (code, outputs) {
  ...
  var code = exports._compile(code, "<eval>");

  return eval("(function (callback) {" +
      code +
      "if (callback) { callback(" + (outputs.join(", ")) + "); }" +
    "});");
};
